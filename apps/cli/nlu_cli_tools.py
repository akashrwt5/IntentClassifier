#!/usr/bin/env python3
"""
Interactive multi-turn NLU demo — the on-device Dialogflow replacement with tool calling.

Usage:
    python apps/cli/nlu_cli_tools.py
"""
...
from nlu_tool_dispatch import default_registry  # noqa: E402
...
def render(r, engine, text):
    ...
    if r.type == "FULFILL":
        ...
        # Dispatch the tool
        tool_res = default_registry.dispatch(r)
        if tool_res.success:
            print(f"  🔧 Tool executed: {tool_res.message}")
        else:
            print(f"  ❌ Tool execution failed: {tool_res.message}")
    elif r.type == "PROMPT":
        print(f"  ❓ {r.message}")
        print(f"     [{via}]")
        if r.parameters:
...
def main():
    print("=== On-device NLU (Dialogflow replacement with Tool Calling) ===")
