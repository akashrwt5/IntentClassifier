...
def compile_lexicon(lang: str, schema: dict, out: Path) -> list[str]:
    ...
    lex = {
        ...
        # VIK-068. Cancellation cues for an open slot-filling flow. Sorted like
        # the sets above so the bundle is byte-stable across builds.
        "cancel_cues": sorted(schema.get("cancel_cues", [])),
    ...
    # Plural folding table. Generated by `nlu_training.train` from the corpus,
    # shipped like `contractions` so every runtime folds identically — the
    # vocabulary the model was fitted on already reflects it, so a runtime that
    # skips this table tokenises against a vocabulary it no longer matches.
    lemmas_path = lang_dir / "lemmas.json"
    if lemmas_path.exists():
        lex["lemmas"] = json.loads(lemmas_path.read_text(encoding="utf-8"))
