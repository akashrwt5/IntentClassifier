...
3. Update ONLY the affected memory file(s), keeping each single-responsibility:
   ...
   decisions, known-issues. Record new architectural choices as ADRs in
   `decisions.md`; move fixed bugs out of `known-issues.md` (and delete the
   matching `--deselect` line in `.github/workflows/pr.yml` if one existed).
4. Verify every path, command, and number you write actually exists on THIS
   branch. Do not import facts from `feature/production-work`.
5. Keep `CLAUDE.md` small — if project knowledge crept in, move it to memory.
6. Report a short diff summary of what you changed and why. Make no code changes.
