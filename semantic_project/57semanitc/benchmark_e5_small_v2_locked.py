...
CLASSIFIER_PATH = (
    MODEL_DIR / "e5_logistic_classifier.joblib"
)
...
OUTPUT_DIR = (
    ROOT / "v3_57intent_e5_small_v2_locked_benchmark"
)
...
REPORT_PATH = (
    OUTPUT_DIR / "locked_classification_report.txt"
)

PREDICTIONS_PATH = (
    OUTPUT_DIR / "locked_predictions.csv"
)
...
SUMMARY_PATH = (
    OUTPUT_DIR / "locked_benchmark_summary.json"
)
...
def load_labels():
    ...
    raise RuntimeError(
        "Could not find label mapping in label_map.json"
    )
...
def main():
    ...
    if text_col is None:
        ...
        raise RuntimeError(
            f"Text column not found: {list(df.columns)}"
        )
    ...
    predicted_ids = classifier.predict(
        X
    )
    ...
    print(
        f"Embedding time : {encode_time:.4f} sec"
    )

    print(
        f"Classifier time: {inference_time:.4f} sec"
    )

    print(
        f"Classifier rows/sec: "
        f"{len(texts) / inference_time:.2f}"
    )

    print(
        f"Classifier ms/row: "
        f"{(inference_time / len(texts)) * 1000:.4f}"
    )
    ...
    with open(
        ...
        f.write(
            f"Embedding time: {encode_time:.6f} sec\n"
        )

        f.write(
            f"Classifier time: {inference_time:.6f} sec\n"
        )
        ...
        f.write(
            f"Classifier ms/row: "
            f"{(inference_time / len(texts)) * 1000:.6f}\n\n"
        )
    ...
    summary = {
        "status":
            "E5-SMALL-V2 LOCKED BENCHMARK COMPLETE",
        ...
        "classifier_rows_per_second":
            float(
                len(texts) / inference_time
            ),

        "classifier_ms_per_row":
            float(
                (inference_time / len(texts))
                * 1000
            ),
