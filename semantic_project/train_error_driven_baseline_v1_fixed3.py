...
class Model(nn.Module):
    ...
    def forward(self,x):
        m=x.eq(PAD); pos=torch.arange(x.size(1),device=x.device).unsqueeze(0); h=self.encoder(self.embedding(x)+self.position(pos),src_key_padding_mask=m); v=(~m).unsqueeze(-1).float(); h=(h*v).sum(1)/v.sum(1).clamp(min=1); return self.classifier(self.norm(h))
