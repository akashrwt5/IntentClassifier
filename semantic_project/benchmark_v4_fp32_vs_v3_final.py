...
V4_CHECKPOINT = ROOT / "e5_v4_student_final" / "v4_e5_distilled_student_fp32.pt"
...
V3_VOCAB_CANDIDATES = [
    ...
    ROOT / "tiny_semantic_student_v3_error_driven" / "vocab.json",
...
def load_csv(path):
    ...
    if "text" not in df.columns:
        raise ValueError(f"{path} does not contain a text/utterance/query column.")
...
def load_v4():
    ...
    if not V4_CHECKPOINT.exists():
        raise FileNotFoundError(f"V4 checkpoint not found:\n{V4_CHECKPOINT}")
...
def load_vocab():
    """
    Resolve the exact project vocab without assuming that
    tiny_semantic_student_v3_balanced/vocab.json exists.
    """
    vocab_path = None

    # Prefer known compact-semantic model locations.
    for candidate in V3_VOCAB_CANDIDATES:
        if candidate.exists():
    ...
    if vocab_path is None:
        found = [
            p
            for p in ROOT.rglob("vocab.json")
            if ".venv" not in str(p) and "v4_vs_v3_benchmark" not in str(p)
        ]

        if found:
            # Prefer semantic-student vocabularies.
            found.sort(
                key=lambda p: (
                    0 if "tiny_semantic_student" in str(p) else 1,
                    len(str(p)),
                )
            )
            vocab_path = found[0]
    ...
    if vocab_path is None:
        raise FileNotFoundError(
            ...
            "Run: find . -name 'vocab.json' -not -path './.venv/*'"
        )

    print("\nResolved V3 vocabulary:")
    print(vocab_path)
    ...
    if not isinstance(obj, dict):
        raise ValueError(f"Unsupported vocab.json format: {vocab_path}")

    print(f"Vocabulary size: {len(obj)}")

    if len(obj) != 895:
        print(
            "WARNING: V3 project expected 895 vocabulary entries; " f"resolved file has {len(obj)}."
        )
...
def v3_tokenize_text(text, vocab, max_len=24):
    ...
    unk = vocab.get("<unk>") if "<unk>" in vocab else vocab.get("[UNK]", 0)

    pad = vocab.get("<pad>") if "<pad>" in vocab else vocab.get("[PAD]", 0)

    cls = vocab.get("<cls>") if "<cls>" in vocab else vocab.get("[CLS]")

    sep = vocab.get("<sep>") if "<sep>" in vocab else vocab.get("[SEP]")
...
def evaluate_classification(
    ...
    if "intent" not in df.columns:
        ...
        print("No intent column; classification " "accuracy/F1 skipped.")

        return {}

    label_to_id = {label: i for i, label in enumerate(labels)}

    usable = df["intent"].isin(label_to_id)
    ...
    if not usable.all():
        ...
        print(f"WARNING {name}: " f"{len(unknown)} unknown labels: {unknown}")

    df2 = df.loc[usable].reset_index(drop=True)

    if len(df2) == 0:
        return {}

    y = np.asarray([label_to_id[x] for x in df2["intent"]])

    v4_pred = v4_logits[usable.to_numpy()].argmax(1)

    v3_pred = v3_logits[usable.to_numpy()].argmax(1)
    ...
    print(f"V3 accuracy : {v3_acc*100:.2f}%")

    print(f"V4 accuracy : {v4_acc*100:.2f}%")

    print(f"Delta       : {(v4_acc-v3_acc)*100:+.2f} pp")

    print(f"V3 Macro F1 : {v3_f1*100:.2f}%")

    print(f"V4 Macro F1 : {v4_f1*100:.2f}%")
...
def evaluate_ood(
    ...
    for threshold in [
        ...
        v3_reject = (v3_conf < threshold).mean()

        v4_reject = (v4_conf < threshold).mean()
        ...
        result[str(threshold)] = {
            "v3_rejection": float(v3_reject),
            "v4_rejection": float(v4_reject),
        }
...
def main():
    ...
    if not UNSEEN.exists():
        raise FileNotFoundError("The locked 595-row unseen CSV was not found.")
    ...
    if len(unseen) != 595:
        ...
        raise RuntimeError("CRITICAL: locked unseen set is not 595 rows. " f"Found {len(unseen)}.")

    texts = unseen["text"].tolist()
    ...
    print("Running V3 on locked unseen...")
    ...
    if v3_unseen.shape[1] != len(labels):
        ...
        raise RuntimeError(
            "V3/V4 class count mismatch: " f"V3={v3_unseen.shape}, " f"V4={v4_unseen.shape}"
        )
    ...
    out_dir = ROOT / "v4_vs_v3_benchmark"
    ...
    summary_path = out_dir / "v4_vs_v3_summary.json"
    ...
    v3_probs = softmax(v3_unseen)

    v4_probs = softmax(v4_unseen)

    detail = unseen.copy()

    detail["v3_pred"] = [labels[i] for i in v3_probs.argmax(1)]

    detail["v4_pred"] = [labels[i] for i in v4_probs.argmax(1)]

    detail["v3_confidence"] = v3_probs.max(1)
    detail["v4_confidence"] = v4_probs.max(1)

    detail["v3_correct"] = detail["v3_pred"] == detail["intent"]

    detail["v4_correct"] = detail["v4_pred"] == detail["intent"]

    detail["regression"] = np.select(
        [
            detail["v3_correct"] & ~detail["v4_correct"],
            ~detail["v3_correct"] & detail["v4_correct"],
        ],
        [
            "V3_ONLY",
    ...
    detail.to_csv(
        out_dir / "unseen_595_details.csv",
        index=False,
    )
    ...
    print(out_dir / "unseen_595_details.csv")

    print("\nIMPORTANT:")

    print("V3 was NOT modified.")

    print("V4 was NOT exported.")

    print("No training occurred.")

    print("No threshold fitting occurred.")

    print("The locked 595-row unseen set was evaluation-only.")

    print("\nNEXT:")

    print("Review V4 regressions before ONNX export.")
