...
# New error-driven candidate built from the locked baseline.
MODEL_V1_DIR = os.path.join(
    SCRIPT_DIR,
    "tiny_semantic_student_error_driven_v1"
)

V1_WEIGHTS = os.path.join(
    MODEL_V1_DIR,
    "student_error_driven_v1_fp32.pt"
)
...
def load_pytorch_model(model_dir):
    ...
    # V1 uses a different filename; V5 models keep student_fp32.pt.
    v1_weights = os.path.join(
        model_dir,
        "student_error_driven_v1_fp32.pt"
    )
...
if not os.path.exists(V1_WEIGHTS):
    raise FileNotFoundError(
        "Error-Driven V1 candidate not found:\n"
        + V1_WEIGHTS
        + "\n\n"
        "Run train_error_driven_baseline_v1_fixed3.py first."
    )

models["Error-Driven V1"] = load_pytorch_model(
    MODEL_V1_DIR
)
...
# Current locked baseline from the previous benchmark.
BASELINE_UNSEEN = 94.29
BASELINE_CONTEXTUAL = 90.62
BASELINE_OOD = 34.38

v1_row = comparison[
    comparison["Model"] == "Error-Driven V1"
]

if not v1_row.empty:
    v1_unseen = float(
        v1_row.iloc[0]["Unseen"].replace("%", "")
    )
    v1_contextual = float(
        v1_row.iloc[0]["Contextual"].replace("%", "")
    )
    v1_ood = float(
        v1_row.iloc[0]["OOD"].replace("%", "")
    )

    print("\n")
    print("=" * 80)
    print("ERROR-DRIVEN V1 VS LOCKED BASELINE")
    print("=" * 80)

    print(
        f"Unseen     : baseline {BASELINE_UNSEEN:.2f}% -> "
        f"V1 {v1_unseen:.2f}% "
        f"({v1_unseen - BASELINE_UNSEEN:+.2f} pp)"
    )

    print(
        f"Contextual : baseline {BASELINE_CONTEXTUAL:.2f}% -> "
        f"V1 {v1_contextual:.2f}% "
        f"({v1_contextual - BASELINE_CONTEXTUAL:+.2f} pp)"
    )

    print(
        f"OOD @ .70  : baseline {BASELINE_OOD:.2f}% -> "
        f"V1 {v1_ood:.2f}% "
        f"({v1_ood - BASELINE_OOD:+.2f} pp)"
    )

    if (
        v1_unseen >= BASELINE_UNSEEN
        and v1_contextual > BASELINE_CONTEXTUAL
        and v1_ood >= BASELINE_OOD
    ):
        print("\nSTATUS: V1 PASSES THE INITIAL FULL-BENCHMARK GATE")
        print("Next: run the targeted regression/typo suite and")
        print("then export V1 to ONNX + INT8.")
    else:
        print("\nSTATUS: KEEP CURRENT INT8 BASELINE")
        print("V1 does not yet pass all three baseline gates.")
...
pd.DataFrame(
    ...
    os.path.join(
        SCRIPT_DIR,
        "semantic_model_contextual_details.csv"
    ),
    index=False
)
...
pd.DataFrame(
    ...
    os.path.join(
        SCRIPT_DIR,
        "semantic_model_ood_details.csv"
    ),
    index=False
)

print("\nSaved:")
print("semantic_model_comparison.csv")
print("semantic_model_contextual_details.csv")
print("semantic_model_ood_details.csv")
