...
class Model(nn.Module):
    def __init__(self):
        super().__init__(); self.e=nn.Embedding(len(vocab),ED,padding_idx=PAD); self.p=nn.Embedding(ML,ED); layer=nn.TransformerEncoderLayer(ED,NH,FF,dropout=.1,activation='gelu',batch_first=True,norm_first=True); self.x=nn.TransformerEncoder(layer,NL); self.n=nn.LayerNorm(ED); self.c=nn.Sequential(nn.Linear(ED,ED),nn.GELU(),nn.Dropout(.1),nn.Linear(ED,len(labels)))
    def forward(self,x):
