...
def discover_csv():
    ...
    for p in ROOT.rglob("*.csv"):
        ...
        has_text = bool(
            cols & {"text", "utterance", "query", "sentence"}
        )
        has_label = bool(
            cols & {"intent", "label", "target", "expected_intent"}
        )
    ...
    if not candidates:
        raise FileNotFoundError(
            "No training CSV auto-discovered. "
            "Pass --data /path/to/train.csv"
        )
    ...
    print("\nAuto-discovered candidate CSVs:")
...
def load_data(path: Path):
    ...
    df = df[df["intent"].isin(LABELS)].copy()
    ...
    if len(df) < 500:
        raise RuntimeError(
            ...
            "Refusing to train on a suspiciously small dataset."
        )
