...
| ND-2 | ✅ **APPROVED 2026-07-14 — executing.** M0 done (oracle frozen). M1 done (engine → packages/runtime/nlu_engine + shim). M2 done (trainers/data-scripts → packages/buildtime/nlu_training; exports → nlu_export; CLIs + unknown_log → apps/cli; Makefile targets updated; dev/debug/test scripts stay in scripts/ for M4). M3 done (schema/entities/localization → content/; training CSVs + multilingual/data → datasets/; data/ keeps only runtime logs; DVC wiring = remaining sub-item pending toolchain + remote decision). Every phase: 60/60 tests + evaluate byte-identical to oracle. M4 (cleanup, shim retirement, dev-script sweep) next. | — |
