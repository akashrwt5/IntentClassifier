...
Type commands like a user would. The engine tracks conversation state, asks
follow-up questions for missing slots, and confirms actions — all offline.

Examples to try:
    set a reminder                     → asks what + when
    remind me to take medication at 5pm→ fills both slots in one shot
    change memory                      → asks which memory
    send a message                     → asks yes/no to confirm
    how is the weather                 → GenAI fallback
"""
