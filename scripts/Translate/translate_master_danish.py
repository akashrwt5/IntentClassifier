...
def post_process_text(text):
    if not isinstance(text, str):
        return text
    # Restore placeholders to their official Danish terminology
    text = text.replace('__EDGEMODE__', 'Edge Mode')
    text = text.replace('__THRIVESCORE__', 'wellness-resultat')
    text = text.replace('__INTELLIVOICE__', 'IntelliVoice')
    text = text.replace('__SELFCHECK__', 'Selvtjek')
    text = text.replace('__HEARSHARE__', 'HearShare')
    text = text.replace('__HCA__', 'Hearing Care Anywhere')
    text = text.replace('__BRAINSCORE__', 'engagement-resultat')
    text = text.replace('__BODYSCORE__', 'aktivitets-resultat')
    text = text.replace('__THRIVEASSISTANT__', 'Thrive Assistent')
    
    # Fix generic "aids" translation errors to ensure NLU accuracy in Danish
    def replace_aids(match):
        prefix = match.group(1)
        prefix_lower = prefix.lower()
        if prefix_lower in ['mit', 'et']:
            return f'{prefix} høreapparat'
        elif prefix_lower in ['mine']:
            return f'{prefix} høreapparater'
        elif prefix_lower in ['de']:
            return 'høreapparaterne'
        return f'{prefix} høreapparater'
        
    text = re.sub(r'\b(mine|mit|et|de)\s+aids\b', replace_aids, text, flags=re.IGNORECASE)
    text = re.sub(r'\baids\b', 'høreapparater', text, flags=re.IGNORECASE)
    
    # Clean up generic "apparater" (devices) references to be explicit
    text = re.sub(r'\bmine\s+apparater\b(?!\s+høre)', 'mine høreapparater', text, flags=re.IGNORECASE)
    text = re.sub(r'\bmit\s+apparat\b(?!\s+høre)', 'mit høreapparat', text, flags=re.IGNORECASE)
    text = re.sub(r'\bet\s+apparat\b(?!\s+høre)', 'et høreapparat', text, flags=re.IGNORECASE)
    text = re.sub(r'\bde\s+apparater\b(?!\s+høre)', 'høreapparaterne', text, flags=re.IGNORECASE)
